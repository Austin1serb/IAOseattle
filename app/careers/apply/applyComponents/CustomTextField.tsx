        ...
        <div className="flex w-full flex-col">
            <div className="relative mt-4 bg-[#f0f0f0] hover:bg-gray-200 rounded-t-[4px] transition-colors duration-200 group">
                <label
                    htmlFor={label}
                    id={`${label}-label`}
                    className={`absolute w-full h-full text-lg transition-all duration-200 transform ${isFocused || value
                ...
                <div className='relative flex items-center justify-center'>
                    <input
                        ...
                        onBlur={handleBlur}
                        required={required}
                        className="block leading-[90px] w-full px-3 pt-8 text-xl min-h-16 max-h-16 group-hover:border-gray-400 text-gray-700 placeholder-transparent bg-transparent border-b-2 border-gray-300 focus:outline-none focus:ring-0 relative transition-colors duration-200"
                    />
                    <div className={`absolute  right-0 m-4  text-primary-dark transition-opacity duration-500 ${hasValidated && !errorMessage ? 'opacity-100 ' : ('opacity-0')}`}>
