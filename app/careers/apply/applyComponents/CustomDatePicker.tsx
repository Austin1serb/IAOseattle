            ...
            <div className="relative mt-4 bg-[#F0F0F0] hover:bg-gray-200 rounded-t-[4px] transition-colors group  duration-200">
                <label
                    ...
                    className={`absolute text-lg transition-all duration-200 transform ${isFocused || value || type === 'date'
                        ? `translate-y-4 text-sm -top-2.5 left-3 text-start ${isFocused ? 'text-blue-600' : 'text-gray-500'} font-light`
                ...
                <div className="date-picker relative w-full">
                    <DatePicker
                        ...
                        required={required}
                    />
                    <div className={`absolute right-2.5 top-4 pointer-events-none text-blue-500 hover:text-blue-600 bg-slate-200 p-2 rounded-full transition-colors duration-300 group-hover:bg-gray-100 z-10 flex items-center justify-center  ${isValid && hasValidated ? 'text-primary-dark' : !isValid && 'text-error'}`}>
                        {/* Calendar Icon */}
                ...
                <div
                    className={`absolute animate-pulse left-0 bottom-0 w-full h-[2.5px] bg-error transition-transform ${isValid && 'hidden'} duration-500 ease-in-out ${!isValid ? 'scale-x-100' : 'scale-x-0'
                        }`}
