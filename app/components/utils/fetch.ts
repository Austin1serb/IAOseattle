...
async function fetchOrgChartRecords(airtableApiKey: string, baseId: string, tableName: string, offset = "") {
	...
	try {
		...
		if (!response.ok) {
			...
			return { records: [], offset: "" }; // Return empty structure instead of throwing
		}
...
export async function fetchAllOrgChartRecords(airtableApiKey: string, baseId: string, tableName: string) {
	let allRecords: any[] = [];
	let offset = "";
	let hasMore = true;
...
export async function fetchAllRecordsForCurrentMonth(airtableApiKey: string, baseId: string, tableName: string) {
	let allRecords: any[] = [];
	let offset = "";
	let hasMore = true;
