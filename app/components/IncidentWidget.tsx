...
export async function getIncidentData() {
	try {
		...
		if (!response.ok) {
			console.warn(`Failed to fetch incident data: ${response.status} ${response.statusText}`);
		...
		datasets: [
			{
				...
				backgroundColor: shuffledTypes.map(([key]) => colorMapping[key]?.background || "#2FE7A6"),
				borderColor: shuffledTypes.map(([key]) => colorMapping[key]?.border || "#0F75E0"),
				borderWidth: 1,
			},
		],
	};
...
const getShortName = (name: string) => {
	const nameMapping: { [key: string]: string } = {
		...
		"Fire / Arson": "Arson",
		"Maintenance Emergency (Broken Water Main / Gas Leak / Electrical Hazard)": "Maintenance",
	};
