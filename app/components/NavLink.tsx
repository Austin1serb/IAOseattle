...
const NavLink: React.FC<NavLinkProps> = ({ href, label, className = "", onClick }) => {
	...
	const combinedClassName = `${className} h-10 w-24 text-center py-2 rounded-md font-medium uppercase transition-colors duration-300 ${
		isActive ? "animate-shine text-blue-600 hover:text-blue-800 shadow-md" : "hover:animate-under-shine text-white hover:text-blue-600 hover:bg-[#ffffff64]"
	}`;
	...
	return (
		<Link
			onClick={handleClick}
			href={href}
			className={`${combinedClassName} ${isPending ? "pointer-events-none" : ""}`}
